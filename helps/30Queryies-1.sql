...
with cliente_reparaciones as (
  ...
  from
    (
      ...
      from
        orden_reparacion
        join revision on id = orden_reparacion_id
      where
...
# 2 Mostrar la informacion de las promociones aplicadas en el departamento mecanico en un periodo de fechas especifico.
  with servicios_entre as (
    ...
    from
      tarea
      join orden_reparacion on id = orden_reparacion_id
    where
      fecha_inicio between cast('1900-01-01' as date)
      and cast('2050-01-01' as date)
  ),
  promociones_usadas as (
    select
      distinct clave_promocion,
      dno
    from
      servicio_requerido
      join tarea on id = servicio_requerido_id
    where
      id in (
        select
          *
...
# 3 Mostrar la informacion del inventario de refacciones que tiene cada producto registrado.
  ...
  compatibilidad as (
    ...
    from
      ...
      join tipo_automovil
    where
      tipo_automovil_id = id
  )
select
  *
from
  compatibilidad
  join refaccion on id = id_refaccion;
# 4 Mostrar las ventas obtenidas con respecto al serivicio de solo diagnosticos.
  with info_diagnosticos as (
    ...
    from
      diagnostico
      join tipo_diagnostico on clave_tipo = clave
    where
      diagnostico.id in (
        select
          *
        from
          revision
        where
          revision.orden_reparacion_id is null
      )
  )
select
  count(precio) as cantidad_diagnosticos,
  sum(precio) as ingresos
from
  info_diagnosticos;
#interpretacion 2 de la 4
  with info_diagnosticos as (
    select
      precio,
      clave,
...
select
  nombre,
  clave,
  count(nombre) as cantidad_diagnosticos,
  sum(precio) as ingresos
from
  info_diagnosticos
group by
...
# 5 Mostrar entre  el periodo de rango de fechas el numero de atenciones para cada departamento del taller.
  with atenciones_periodo as (
    ...
    from
      tarea
      join orden_reparacion on servicio_requerido_id = id
    where
...
select
  nombre,
  dno,
  count(fecha_finalizacoin) as cantidad
from
  atenciones_periodo
...
# 6 Mostrar las ganacias obtenidas en un rango de fechas especifico en base a un despartamento en particular.
  ...
  ingresos_tareas as (
    ...
    from
      tareas_departamento
      join costo_servicios on id = servicio_requerido_id
  )
select
  dno,
...
# 7 Mostrar el empleado que mayor ganancia obtuvo en un rango de fechas especifico.
  ...
  servicios_realizados as (
    ...
    from
      tareas_empleados
      join servicio_requerido on id = servicio_requerido_id
  ),
  ingresos_empleados as (
    select
      ANY_VALUE(encargado) as encargado,
      sum(costo) as ganancias
    from
  ...
  empleado_ganancias as (
    select
      ANY_VALUE(encargado) as encargado,
      ANY_VALUE(ganancias) as ganancias
    from
      ingresos_empleados
  ),
  max_ganancias as (
...
from
  empleado
  join empleado_ganancias on encargado = dni
  and ganancias =(
    select
      m
    from
...
from
  empleado
  join supervisores on supervisor = super_dni
order by
  super_dni asc;
...
from
  cliente
  join automovil on cliente_dni = dni
...
# 11 Mostrar el departamento que presenta el mayor consumo de refacciones en un periodo de fecha determinado y muestr el numero de refacciones utilizadas.
  ...
  tareas_periodo as (
    select
      servicio_requerido_id as sr_id,
      dno
    from
      tarea
      join orden_reparacion on id = servicio_requerido_id
  ...
  ordenes_departamento as (
    select
      ANY_VALUE(dno) as dno,
      sum(total) as numero_total
    from
      servicio_requerido
      join tareas_periodo on sr_id = id
      join ordenes_piezas_cantidad on refacciones_necesarias_id = pedido_refaccion_id
    group by
      dno
  ),
  max_dep as (
    select
      ANY_VALUE(dno) as dno,
      ANY_VALUE(numero_total) as numero_ordenadas
    from
      ordenes_departamento
    having
      max(numero_total)
  )
select
...
from
  departamento
  join max_dep on dno = numero;
# 12 Mostrar las promociones vigentes por un rango de fechas determinado.
set
  @inicio = cast('1900-01-01' as date);
set
  @final = cast('2050-01-01' as date);
select
  *
from
  promocion
...
having
  max(cantidad);
# la 24 la hizo marcos yo no fui hector no me saques del equipo
